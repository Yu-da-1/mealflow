...
> [!CAUTION]
> **このステップは quality-checker サブエージェントの呼び出しが唯一の完了手段。**
> `pnpm tsc --noEmit` や `pnpm eslint` の手動実行は **代替にならない。**
> quality-checker を呼び出さずに Record へ進むことは禁止。

### ステップ 1: quality-checker を呼び出す（必須・省略不可）

以下のプロンプトをそのままサブエージェントとして実行する。

> quality-checker を使って実装したコードを検証してください

### ステップ 2: 完了報告を受け取る（ゲート条件）

quality-checker が以下の形式で完了報告を返すまで Record ステップに進んではならない。
...
### チェックリスト（全項目が ✅ になるまで Record に進まない）

- [ ] quality-checker サブエージェントを呼び出した（手動コマンドのみで済ませていない）
- [ ] quality-checker から「quality-check 完了」の完了報告を受け取った
- [ ] tsc: グリーン
- [ ] eslint: グリーン
- [ ] vitest: グリーン
